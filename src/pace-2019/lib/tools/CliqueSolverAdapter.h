...
std::vector<int> solveMISInstanceWithCliqueSolver(std::vector<std::vector<int>> &MISInstance, long limit, bool docheck=false) {
    ...
    for(int i = 0; i < numVertices; i++) {
        ...
        for(const auto &neighbor: MISInstance[i]) {
					// std::cout << neighbor << " ";
					// if (!adjMatrix[i][neighbor]) std::cout << "!!!";
    ...
    for(int i = 0; i < numVertices; i++) {
		...
		//=====================================================
		// Note: It's possible that numEdges > numEdgesReal
		//       because MISInstance[i] may have duplicates.
		//=====================================================
    // if(numEdges != numEdgesReal) {
		// 	  std::cout << "numVertices=" << numVertices << ", numEdges=" << numEdges << ", numEdgesReal=" << numEdgesReal << std::endl;
		// 		// std::cout << MISInstance << std::endl;
    ...
    build_simple_graph_instance_from_pointer_to_edge_list(edgesFrom.data(), edgesTo.data(), numVertices, numEdgesReal);
